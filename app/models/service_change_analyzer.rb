class ServiceChangeAnalyzer
  ...
  def self.service_change_summary(route_id, actual_routings, scheduled_routings, recent_scheduled_routings, timestamp)
  ...
  def self.match_route(current_route_id, reroute_service_change, recent_scheduled_routings, timestamp)
  ...
  def self.current_routings(timestamp)
    if !@current_routings || !@timestamp || (timestamp - @timestamp) > 60
      ...
      puts "Use current routings at #{Time.at(timestamp)}"
      @current_routings
    end
  end
  ...
  def self.interchangeable_transfers
