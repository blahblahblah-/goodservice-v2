class DelayNotification
  ...
  def match_routing?(routing, potential_matched_stops, potential_stop_tracks)
    return false unless routing.each_cons(stops.size).any? { |arr| arr == stops && arr.all? { |s| !tracks[s] || !potential_stop_tracks[s] || tracks[s].any? { |t| potential_stop_tracks[s] == t }}}
    return true if stops.any? { |s| potential_matched_stops.include?(s) && (!tracks[s] || !potential_stop_tracks[s] || tracks[s].any? { |t| potential_stop_tracks[s] == t })}
